...
**O loop local de retry de variante de telefone que a rota tem hoje é removido.** Os providers já cuidam disso internamente (`MetaProvider` por dentro; `EvolutionProvider` nem precisa) — esse é o ganho concreto de consolidar pela costura, e resolve a duplicação sinalizada nas revisões das Fases 1/2.

...
describe("mondayIndex", () => {
  // new Date(year, monthIndex, day) — the local-time constructor, not
  // a bare "YYYY-MM-DD" string. A date-only ISO string parses as UTC
  // midnight (spec behavior); mondayIndex reads getDay(), which is
  // local-time. On any machine west of UTC that mismatch silently
  // shifts the date back a day, misidentifying the weekday — this
  // constructor form sidesteps it regardless of the runner's timezone.

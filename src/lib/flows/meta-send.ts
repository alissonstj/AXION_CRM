...
export async function engineSendText(
  ...
  // Provider resolution is the seam — it fetches + decrypts the
  ...
  const waMessageId = result.providerMessageId
  const workingPhone = result.workingRecipient ?? sanitized
...
async function sendInteractiveViaMeta(
  ...
  // Provider owns the phone-variant retry internally now (same
  // behavior previously duplicated here and in automations/meta-send.ts).

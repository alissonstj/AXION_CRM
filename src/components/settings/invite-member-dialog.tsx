...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our AXION CRM PRO account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our AXION CRM PRO account"
    // wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'our AXION CRM PRO account';

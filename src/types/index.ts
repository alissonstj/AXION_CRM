...
export interface Contact {
  ...
  user_id: string;
  account_id: string;
  /** Null for a contact WhatsApp only ever addressed by @lid, with no
   *  phone-number alt provided (migration 051) — see `lid`. */
  phone: string | null;
  /** Digits-only form of `phone`, generated by the DB (migration 022)
   *  and unique per account. Read-only. */
  phone_normalized?: string;
  /** The contact's WhatsApp LID form, when known (migration 049).
...
export interface Conversation {
  id: string;
  user_id: string;
  /** Null for a group conversation (migration 046) — a group has no
   *  single contact. */
  ...
  updated_at: string;
  contact?: Contact;
  /** WhatsApp group support (migration 046). `is_group` distinguishes a
   *  group thread; the group's identity lives in these fields instead of
   *  `contact`. */
  is_group?: boolean;
  group_jid?: string | null;
  group_name?: string | null;
  group_avatar_url?: string | null;
  /** Set (a few seconds in the future) whenever the CUSTOMER's WhatsApp
   *  reports a "composing" presence; cleared to null on any other
   *  presence value. Time-boxed rather than a plain boolean so a missed
   *  "stopped typing" event can't leave "digitando…" stuck on forever —
   *  read it as `typing_until > now()`, never as a bare truthy check.
   *  Migration 049. */
...
export interface Message {
  ...
  /**
   * Group messages only (migration 046): the participant who sent this
   * message within the group — name + phone. Null on 1:1 messages and on
   * our own (agent) group messages. Drives the sender label shown above
   * a group bubble, like WhatsApp.
   */

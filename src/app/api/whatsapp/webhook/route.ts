...
async function processWebhook(body: { entry?: WhatsAppWebhookEntry[] }) {
  ...
  for (const entry of body.entry) {
    for (const change of entry.changes) {
      ...
      // Translate this change's raw Meta payload into the
      // provider-agnostic NormalizedInbound shape (Task 1/3). Scoped to
      // just this change — parseWebhook itself has no notion of which
      // whatsapp_config a message belongs to, so passing the whole raw
      // body here would fan out messages meant for other numbers/accounts
      // into this config's ingest calls.
      const provider = new MetaProvider({
        phoneNumberId,
        accessToken: decryptedAccessToken,
      })
      const inbounds = provider.parseWebhook({
      ...
      for (let i = 0; i < inbounds.length; i++) {
        ...
        // Media resolution stays here — provider-specific. parseWebhook
        // already built the unverified `/api/whatsapp/media/<id>` proxy
        // URL; verify the media_id against Meta before trusting it. A
        // failed verification drops the media but the message itself
        // still gets ingested (matches the pre-refactor behavior).
        if (inbound.mediaUrl && message) {
          const mediaId = mediaIdForMessage(message)
          inbound.mediaUrl = mediaId
            ? await verifyAndBuildUrl(mediaId, decryptedAccessToken)
            : null
        }
 ...
 */
async function verifyAndBuildUrl(
  mediaId: string,
  accessToken: string
): Promise<string | null> {
  try {
...
/** Pulls the Meta media id off whichever type-specific field carries
 *  it, mirroring the mapping MetaProvider.parseWebhook uses to build
 *  the (unverified) proxy URL. */
function mediaIdForMessage(message: WhatsAppMessage): string | undefined {
  switch (message.type) {
    case 'image':

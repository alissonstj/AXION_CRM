...
const { mockSendTyping, providerState } = vi.hoisted(() => ({
...
beforeEach(() => {
  ...
  vi.mocked(getChannelForAccount).mockReset().mockImplementation(
    async () => ({ id: providerState.id, sender: { sendTyping: mockSendTyping } }) as never,
  );
...
describe('POST /api/whatsapp/typing', () => {
  it('fires sendTyping with the last customer message id as context, a 5.5s duration', async () => {
    const res = await POST(req({ conversation_id: 'conv-1' }));
    expect(res.status).toBe(200);
    expect(mockSendTyping).toHaveBeenCalledWith({
      to: '15551234567', contextProviderMessageId: 'WAMID-LAST', durationMs: 5500,
    });
  });
  ...
  it('still fires sendTyping (contextProviderMessageId: undefined) when there is no prior customer message — Evolution does not need it', async () => {
    mockLastCustomerMessage = null;
    const res = await POST(req({ conversation_id: 'conv-1' }));
    expect(res.status).toBe(200);
    expect(mockSendTyping).toHaveBeenCalledWith({
      to: '15551234567', contextProviderMessageId: undefined, durationMs: 5500,
    });
  });

  it('does not block the response on sendTyping — responds before the provider call settles', async () => {
    let resolveSendTyping: () => void = () => {};
    mockSendTyping.mockReturnValue(new Promise<void>((resolve) => { resolveSendTyping = resolve; }));
    const res = await POST(req({ conversation_id: 'conv-1' }));
    expect(res.status).toBe(200);
    expect(mockSendTyping).toHaveBeenCalled();
    resolveSendTyping();
  ...
  it('still returns 200 when the provider call itself rejects — best-effort, never surfaced as an error', async () => {
    mockSendTyping.mockRejectedValue(new Error('Evolution API error: 500'));
    const res = await POST(req({ conversation_id: 'conv-1' }));
    expect(res.status).toBe(200);
    const json = await res.json();

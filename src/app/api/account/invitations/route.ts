...
//   gets today's permissive behavior.
//
// Previous implementations hard-defaulted to a third-party marketing
// domain here. Forks that didn't set `NEXT_PUBLIC_SITE_URL` got
// invite links pointing at someone else's site, which 404s on
// `/join/<token>`. Falling back to an empty string instead makes the
// broken link obvious (missing host) rather than silently wrong.
...
function getBaseUrl(request: Request): string {
  ...
  if (allowList && (forwardedHost || host)) {
    ...
    console.warn(
      "[POST /api/account/invitations] could not derive base URL from request; set NEXT_PUBLIC_SITE_URL to fix",
    );
  }
  return "";
}

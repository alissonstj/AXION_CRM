...
// Spec-defined seed — color + position per the product spec; the name
// is resolved from Pipelines.page.defaultStageNames at seed time (see
// seedDefaultPipeline) so a new account's default board matches the
// signed-in user's locale instead of always seeding in English.
const SPEC_DEFAULT_STAGE_SHAPE = [
...
export default function PipelinesPage() {
  ...
  const seedDefaultPipeline = useCallback(async (): Promise<Pipeline | null> => {
    ...
    const stagesPayload = SPEC_DEFAULT_STAGE_SHAPE.map((s) => ({
      pipeline_id: pipeline.id,
      name: t(`defaultStageNames.${s.nameKey}`),
      color: s.color,
      position: s.position,
    }));
